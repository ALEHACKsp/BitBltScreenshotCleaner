...
BOOL WINAPI hkBitBlt(HDC hdcDst, int x, int y, int cx, int cy, HDC hdcSrc, int x1, int y1, DWORD rop)
{
	...
	if (sshotCleaner.bOverlay)
	{
		...
		if (hOverlayWindow)
		{
			...
			Log.Log(L"Overlay Found - Screenshot Cleaned");
		}

		else
		{
			//if no overlay, we already sent the regular screenshot
			Log.Log(L"Overlay not found - Uncleaned(possibly dirty) Screenshot Sent");
		}
	}
	...
	else if (!sshotCleaner.bOverlay && *sshotCleaner.pDrawEnabled == true)
	{
		...
		*sshotCleaner.pDrawEnabled = true;
		Log.Log(L"Internal Drawing - Screenshot Cleaned");
	}

	else Log.Log(L"Drawing not detected, Uncleaned(possibly dirty) Screenshot Sent");
...
bool SShotCleaner::Init()
{
	if (bOverlay)
	{
		...
		if (mem::SetDebugPrivilege(true)) //TODO: potential problems could be caused here
		{
